...
static const wchar_t* GetString(int lang, const wchar_t* key) {
    ...
    static const wchar_t* strings_traditional[] = {
        L"設定", L"功能開關", L"建立文字檔案", L"開啟記事本", L"開機自動啟動",
        L"快速鍵設定", L"建立文字檔案", L"開啟記事本", L"開關-建立檔案", L"開關-開啟記事本",
        L"清除", L"重設", L"重設全部", L"語言", L"設定", L"關於", L"退出",
        L"【文字檔案建立工具】\r\n一個能用快速鍵快速建立新文字檔案的自製小工具。\r\n也能用快速鍵快速開啟記事本。\r\n\r\n可在本軟體設定中自訂快速鍵。\r\n\r\n版權：Torin Linsley\r\n開放原始碼授權：LGPL v2.1\r\n\r\n項目連結：https://github.com/TorinLinsley/Text_Document_Creator",
        L"返回"
    };
    static const wchar_t* strings_english[] = {
        L"Settings", L"Function Switches", L"Create Text File", L"Open Notepad", L"Auto Start",
        L"Shortcut Keys", L"Create Text File", L"Open Notepad", L"Switch-Create File", L"Switch-Open Notepad",
...
static void ToggleAboutMode(HWND hDlg) {
    ...
    HWND hStaticLine = GetDlgItem(hDlg, IDC_STATIC);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_OPEN_NOTEPAD: {
        ...
        for (int i = 0; i < 30; i++) {
            Sleep(50);
            hNotepad = FindWindowW(L"Notepad", NULL);
            if (hNotepad) break;
        }
        ...
        if (hNotepad) {
            ...
            DWORD dwThreadId = GetWindowThreadProcessId(hNotepad, NULL);
            ...
            HWND hEdit = FindWindowExW(hNotepad, NULL, L"Edit", NULL);
            if (hEdit) {
                SetFocus(hEdit);
            }
            else {
            ...
            SetWindowPos(hNotepad, HWND_NOTOPMOST, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE);
        }
        break;
    }
